...
export class StripeHelper extends StripeHelperBase {
  ...
  async verifyPlanUpdateForSubscription(
    ...
    // have a graceful fallback, so we cheat a little by merging the upgrade
    // config values from the Firestore configs into the metadata.
    ...
    if (this.paymentConfigManager) {
      ...
      currentPlan.plan_metadata['productSet'] =
        ...
        currentPlan.plan_metadata!['productSet'];
      currentPlan.plan_metadata['productOrder'] = currentPlan.configuration
        ?.productOrder
        ? `${currentPlan.configuration.productOrder}`
        : currentPlan.plan_metadata!['productOrder'];
      newPlan.plan_metadata ??= {};
      newPlan.plan_metadata['productSet'] =
        newPlan.configuration?.productSet ||
        newPlan.plan_metadata!['productSet'];
      newPlan.plan_metadata['productOrder'] = newPlan.configuration
        ?.productOrder
        ? `${newPlan.configuration.productOrder}`
        : newPlan.plan_metadata!['productOrder'];
    }
  ...
  async extractInvoiceDetailsForEmail(invoice: Stripe.Invoice) {
    ...
    const {
      ...
      downloadURL:
        planConfig.urls?.successActionButton || productMetadata.downloadURL,
      successActionButtonURL:
  ...
  async extractSubscriptionUpdateEventDetailsForEmail(event: Stripe.Event) {
    ...
    const planConfig = this.maybeGetPlanConfig(planIdNew);
